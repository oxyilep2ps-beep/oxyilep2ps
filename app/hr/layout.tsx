...
import { isAdminEmail, isSuperHrEmail } from '@/lib/auth/routing';

const links = [
  { href: '/hr', label: 'Overview' },
  { href: '/hr/careers', label: 'Careers' },
  { href: '/hr/learning', label: 'Learning Hub' },
  { href: '/hr/blogs', label: 'Blog Editor' },
];
...
export default async function HrLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isAdminEmail(user.email) && !isSuperHrEmail(user.email) && profile?.role !== 'HR' && profile?.role !== 'ADMIN') {
    redirect('/dashboard');
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-7xl px-4 pb-24 pt-8 sm:px-6">
      <header className="glass-card mb-8 rounded-2xl p-6">
        ...
        <h1 className="mt-2 text-2xl font-black text-neutral-950 dark:text-white">People & Content</h1>
