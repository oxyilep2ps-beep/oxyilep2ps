...
export default function PendingVerificationPage() {
  return (
    <div className="relative mx-auto flex min-h-[calc(100dvh-6rem)] max-w-7xl items-center justify-center px-4 py-12 sm:px-6">
      <motion.article
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, ease: 'easeOut' }}
        className="relative z-10 w-full max-w-lg rounded-[2rem] border border-white/70 bg-white/80 p-8 text-center shadow-glass dark:border-white/15 dark:bg-black/75 sm:p-10"
      >
        <motion.div
        ...
        >
          <div className="grid h-20 w-20 place-items-center rounded-2xl bg-brand-500/15 text-brand-500">
            <motion.div
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Link
            href="/"
            className="rounded-full border border-neutral-200/80 bg-white/60 px-6 py-3 text-sm font-semibold text-neutral-700 transition hover:border-brand-300 dark:border-white/15 dark:bg-black/40 dark:text-neutral-200"
          >
            Back to home
          </Link>
          <a
