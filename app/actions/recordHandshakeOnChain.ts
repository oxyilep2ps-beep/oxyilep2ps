...
/**
 * Relayer: hash handshake agreement data and anchor it on Polygon Amoy (0-value tx + data payload).
 * Then sync ACTIVE status + tx hash to Supabase. No MetaMask required on the client.
 */
export async function recordHandshakeOnChain(
  handshakeId: string
): Promise<RecordHandshakeOnChainResult> {
  if (!process.env.POLYGON_PRIVATE_KEY?.trim()) {
    throw new Error('Server Misconfiguration: Polygon Relayer Key is missing.');
  }
  ...
  try {
    ...
    if (!id) {
      return { ok: false, error: 'handshakeId is required' };
    }
    ...
    if (hsError || !handshake) {
      return { ok: false, error: 'Handshake not found' };
    }

    if (!handshake.lender_approved_at || !handshake.borrower_approved_at) {
      return { ok: false, error: 'Both parties must approve before escrow funding' };
    }
    ...
    if (!isValidTxHash(txHash)) {
      return { ok: false, error: 'Polygon relayer returned an invalid transaction hash' };
    }
    ...
    return { ok: true, txHash, agreementHash };
  } catch (e) {
